...
export default function Timeline({ items }) {
  return (
    <div className="relative max-w-4xl mx-auto" role="list" aria-label="المسيرة المهنية">
      {/* خط المنتصف */}
  <div className="absolute right-4 md:right-1/2 top-0 bottom-0 w-0.5 bg-gradient-to-b from-(--gold) via-(--gold)/50 to-transparent transform md:translate-x-1/2" />
      ...
      {items.map((item, index) => {
        ...
        return (
          ...
          >
            {/* نقطة على الخط */}
            <div className="absolute right-4 md:right-1/2 w-4 h-4 rounded-full bg-(--gold) border-4 border-(--cream-light) shadow-md transform translate-x-1/2 mt-6 z-10" />
            ...
            >
              <div className="bg-white rounded-xl p-6 shadow-md hover:shadow-lg transition-shadow duration-300 border border-gray-100">
                <span className="inline-block px-3 py-1 text-xs font-bold text-(--navy) bg-(--gold)/20 rounded-full mb-3">
                  {item.year}
                </span>
